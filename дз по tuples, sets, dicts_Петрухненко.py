...
reviewers = [paul, jane, kate, nick]
names = ['Paul', 'Jaine', 'Kate', 'Nick']
words_number = []
for i in reviewers:
  prepared_review = set(
    i.lower().replace('i\'ve', 'i have').replace('\'s', '').translate(
      str.maketrans('','',string.punctuation)).split()
  )
  words_number.append(len(prepared_review))
ranking = dict(zip(names, words_number))
print(ranking)
print('The best review author is', max(
  ranking, key=ranking.get), 'with',max(ranking.values()), 'unique words.')
...
transliteration_upper = [i.upper() for i in transliteration]
transliteration_upper2 = transliteration.values()
transliteration_upper2 = [i.title() for i in transliteration_upper2]
transliteration_upper = dict(zip(transliteration_upper, transliteration_upper2))
transliteration.update(transliteration_upper)

word = input('Введите слово или фразу для транслитерации: ')
exceptions = ['ъе', 'ье', 'уе', 'ае', 'ое', 'ие', 'ее', 'ые', 'эе', 'яе', 'юе', 'ёе', ' е']
for i in exceptions:
  if i in word:
      word = word.replace(i, i[0] + 'йе')
for i in word.strip('?,."!'):
    if i in transliteration:
        print(transliteration[i], end='')
